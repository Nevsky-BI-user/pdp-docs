# AC.Середня тривалість лікарняного на співробітника
...
Колонки: `EMPLOYEE_ID`, `IS_MAIN_POSITION`, `Index`, `USER_ACCESS_ID`, `sick_leave_day_Total`, `sick_leave_id_cnt`
...
Колонки: `dim_Admin_LT_OS[USER_ACCESS_ID]`, `dim_Admin_OS[USER_ACCESS_ID]`, `fact_Employee_List[EMPLOYEE_ID]`, `fact_Employee_List[IS_MAIN_POSITION]`, `fact_Employee_List[USER_ACCESS_ID]`, `fact_Metrics[sick_leave_day_Total]`, `fact_Metrics[sick_leave_id_cnt]`, `t_HierarchyTypes[Index]`

### Схема

```mermaid
graph LR
  M["AC.Середня тривалість лікарняного на співробітника"]
...
**Бізнес-назва:** Середня тривалість лікарняного на співробітника

### Опис із ТЗ

Розрахункове поле.   Відношення загальної кількості днів лікарняних (крім по вагітності та пологам) до кількості унікальних оформлених лікарняних на команду.   Цифру округлювати до десятих, в більшу сторону, якщо друга цифра після коми 5-9, і в меншу сторону, якщо друга цифра після коми 0-4.

??? note "Поля-джерела та пов'язані бізнес-метрики (9)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `IS_MAIN_POSITION` | Пріоритетне місце роботи · is_main_position |
    | `sick_leave_day_Total` | Середня тривалість лікарняного · Тривалість лікарняного (зважена на ФТЕ) · Середня тривалість лікарняного на співробітника |
