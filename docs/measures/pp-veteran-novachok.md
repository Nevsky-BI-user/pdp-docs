...
**Бізнес-назва:** Ветеран-Новачок

### Опис із ТЗ

Поле зберігається в довіднику dm.`vw_R27_dim_person` ]   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `person_key`.   Якщо 1 - Так, якщо 0 - Ні.

??? note "Поля-джерела та пов'язані бізнес-метрики (2)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `IS_VETERAN_NEWMAN` | Ветеран-Новачок · Кількість ветеранів-новачків |
