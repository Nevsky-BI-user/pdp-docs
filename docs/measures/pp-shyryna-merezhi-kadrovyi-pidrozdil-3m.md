...
//     VALUES(dim_Unit[PERSONNEL_UNIT]),
...
```mermaid
graph LR
  M["PP.Ширина мережі (кадровий підрозділ, 3м)"]
...
Поле зберігається в довіднику `dm.vw_R27_dim_unit`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `division_key` = `unit_key`.   Поле завжди має значення, пусте поле не допускається   Якщо назва не вміщається в одну строку, перенести на іншу

Поле зберігається в довіднику `dm.vw_R27_dim_unit`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`, через відповідний зв’язок за ключем `division_key` = `unit_key`.   Поле завжди має значення, пусте поле не допускається   Якщо назва не вміщається в одну строку, перенести на іншу

Якщо це структурна одиниця, то це її назва (назва кадрового підрозділу). Якщо це lead team керівника, то показати текст "Мої прямі підлеглі".   Поле зберігається в довіднику `dm.vw_R27_dim_unit`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `division_key` = `unit_key`.   Поле завжди має значення, пусте поле не допускається   Якщо назва не вміщається в одну строку, перенести на іншу.

??? note "Поля-джерела та пов'язані бізнес-метрики (6)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `PERSONNEL_UNIT` | Кадровий підрозділ працівника · Підрозділ кадровий · Назва попереднього кадрового підрозділу · Назва поточного кадрового підрозділу · Команда · division_person |
