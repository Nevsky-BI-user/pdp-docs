...
КР змінено, якщо поле `kr_change`= CHANGE

КР без змін, якщо поле `kr_change`= NO CHANGE

КР видалено, якщо поле `kr_change`= DELETE

КР змінено, якщо поле `kr_change`= NEW
