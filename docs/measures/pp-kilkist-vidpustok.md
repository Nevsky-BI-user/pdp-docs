# PP.Кількість відпусток

*тека `Personal_Profile\Здоров'я та благополуччя` · формат `0`*

## Бізнес-суть

IS_MAIN_POSITION → Пріоритетне місце роботи; IS_MAIN_POSITION → is_main_position; Vacation_Number → Кількість відпусток

1 - Так  <br>0 - Ні Розрахункове поле.  <br>Скільки було відпусток за останні 12 місяців.
...
Колонки: `EMPLOYEE_ID`, `IS_MAIN_POSITION`, `USER_ACCESS_ID`, `Vacation_Number`
...
Колонки: `fact_Employee_List[EMPLOYEE_ID]`, `fact_Employee_List[IS_MAIN_POSITION]`, `fact_Employee_List[USER_ACCESS_ID]`, `fact_Metrics[USER_ACCESS_ID]`, `fact_Metrics[Vacation_Number]`

### Схема

```mermaid
graph LR
  M["PP.Кількість відпусток"]
