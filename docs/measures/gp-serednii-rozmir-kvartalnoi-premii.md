# GP.Середній розмір квартальної премії
...
Колонки: `BONUS_QUARTER_SALARY_CNT`, `CALC_TYPE_CODE`, `IS_ACTUAL`, `Index`, `MIN_TARIFF_RATE`, `USER_ACCESS_ID`, `category_name`
...
Колонки: `dim_Admin_LT_OS[USER_ACCESS_ID]`, `dim_Admin_OS[USER_ACCESS_ID]`, `fact_TRS_Plan[BONUS_QUARTER_SALARY_CNT]`, `fact_TRS_Plan[CALC_TYPE_CODE]`, `fact_TRS_Plan[IS_ACTUAL]`, `fact_TRS_Plan[MIN_TARIFF_RATE]`, `fact_TRS_Plan[category_name]`, `t_HierarchyTypes[Index]`

### Схема

```mermaid
graph LR
  M["GP.Середній розмір квартальної премії"]
