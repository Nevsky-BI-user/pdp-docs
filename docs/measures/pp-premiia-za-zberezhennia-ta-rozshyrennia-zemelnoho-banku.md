# PP.Премія за збереження та розширення земельного банку
...
CALCULATE(
	...
	fact_TRS[accrual_types_key]="18f9ddbe-9571-b614-dda3-f65900e3807c",
...
```mermaid
graph LR
  M["PP.Премія за збереження та розширення земельного банку"]
  M --> dim_Date["dim_Date"]
  M --> fact_TRS["fact_TRS"]
```
...
**Бізнес-назва:** Премія за збереження та розширення земельного банку

### Опис із ТЗ

Потрібно відібрати всі записи по працівнику `person_key`, періоду `Period`, організації `organization_key` , підрозділу `division_key`, посаді `position_key`, де  поле `accrual_types_key` = '18f9ddbe-9571-b614-dda3-f65900e3807c'   та `category_of_accrual_sort`  = '2' за останні 12 місяців, НЕ включаючи поточний.   Якщо по працівнику записи відсутні, то показати прочерк "-".   В деталізацію винести дані в табличному вигляді суму та місяць, за який було здійснено нарахування
