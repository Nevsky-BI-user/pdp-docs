# GP.Доля команди із премією МХП Зірки, %
...
```mermaid
graph LR
  M["GP.Доля команди із премією МХП Зірки, %"]
...
**Бізнес-назва:** Доля команди із премією МХП Зірки, %

### Опис із ТЗ

Потрібно підрахувати унікальну кількість працівників у команді на поточний момент, для яких значення `payments_fact_UAH` > 0.00 по  `accrual_types_key` = '9781d4aa-3a0d-1458-623a-7a93e90a2284'    та `category_of_accrual_sort`  = '3' хоча б один раз за останні 12 місяців, НЕ включаючи поточний, та поділити на поточну кількість членів команди.
