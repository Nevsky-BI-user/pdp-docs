# GP.Середній розмір річного бонусу
...
Колонки: `BONUS_YEAR_SALARY_CNT`, `CALC_TYPE_CODE`, `IS_ACTUAL`, `Index`, `MIN_TARIFF_RATE`, `USER_ACCESS_ID`, `category_name`
...
Колонки: `dim_Admin_LT_OS[USER_ACCESS_ID]`, `dim_Admin_OS[USER_ACCESS_ID]`, `fact_TRS_Plan[BONUS_YEAR_SALARY_CNT]`, `fact_TRS_Plan[CALC_TYPE_CODE]`, `fact_TRS_Plan[IS_ACTUAL]`, `fact_TRS_Plan[MIN_TARIFF_RATE]`, `fact_TRS_Plan[category_name]`, `t_HierarchyTypes[Index]`

### Схема

```mermaid
graph LR
  M["GP.Середній розмір річного бонусу"]
...
**Бізнес-назва:** Середній розмір річного бонусу

### Опис із ТЗ

Розрахункове.   Потрібно зсумувати значення поля `BONES_SIZE` по тим працівникам, для яких `BONUS_YEAR_SALARY_CNT`>0, `IS_ACTUAL`  = "1", `CALC_TYPE_CODE` = "UAH", `category_name` = Фіксована винагорода та поділити на кількість членів команди, у яких є такий вид нарахування в планових виплатах.   В деталізацію вивести перелік таких працівників та суму по кожному із них
