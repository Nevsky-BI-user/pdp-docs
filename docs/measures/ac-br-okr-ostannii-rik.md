...
??? note "Поля-джерела та пов'язані бізнес-метрики (4)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `OKR_LAST_YEAR_RATE` | Тренд OKR (%) · OKR останній рік · Значення коефіцієнта індивідуального бонусу за останній період · OKR команди за останній рік |
