# GP.Доля команди з доплатою за роз’їзний характер роботи, %
...
```mermaid
graph LR
  M["GP.Доля команди з доплатою за роз’їзний характер роботи, %"]
...
**Бізнес-назва:** Доля команди з доплатою за роз’їзний характер роботи, %

### Опис із ТЗ

Розрахункове поле.   Потрібно підрахувати кількість працівників у команді для яких по `ACCRUAL_ORG_CODE` = 00193 поле `INIT_PAYMENT_PLAN_SUM`>0, `IS_ACTUAL`  = "1", `END_DATE` > поточна дата, або `END_DATE` = "01.01.2001 та поділити на поточну кількість членів команди.
