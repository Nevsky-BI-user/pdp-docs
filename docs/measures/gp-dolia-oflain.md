...
CALCULATE(
	DIVIDE(
		CALCULATE(
			...
			'fact_Employee_List'[WORK_FORMAT_ON_EMPLOYEE_KEY] = "1"
		),
		COUNTROWS('fact_Employee_List')
	)
)
...
**Бізнес-назва:** Доля офлайн (%)

### Опис із ТЗ

Розрахункове поле: відношення кількості працівників із форматом роботи Офлайн у команді до загальної кількості працівників.   Відношення кількості працівників, для яких `work_format_on_employee_key` = 1 до загальної чисельності команди (метрика Кількість співробітників всього, чол.)

Розрахункове поле: ідношення кількості працівників із форматом роботи Офлайн у команді до загальної кількості працівників.   Відношення кількості працівників, для яких `work_format_on_employee_key` = 1 до загальної чисельності команди (людей)
