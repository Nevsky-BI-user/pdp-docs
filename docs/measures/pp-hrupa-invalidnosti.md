...
**Бізнес-назва:** Група інвалідності

### Опис із ТЗ

Поле зберігається в довіднику ` dm.vw_R27_dim_person`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`  через відповідний зв’язок за ключем `person_key`   Якщо значення в полі відсутнє, то показати текст "Дані відсутні"  або знак "-".   При виводі даних у звіт змапити дані:   ПЕРВАЯ_ГРУППА = 1   ВТОРАЯ_ГРУППА = 2   ТРЕТЬЯ_ГРУППА = 3   РЕБЕНОК_ИНВАЛИД_ДО_18_ЛЕТ = Дитина інвалід до 18р. (теоретично такого значення бути не повинно)
