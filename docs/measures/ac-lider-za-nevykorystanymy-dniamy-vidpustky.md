...
**Бізнес-назва:** Лідер за невикористаними днями відпустки

### Опис із ТЗ

Розрахункове поле.   Відібрати `EMPLOYEE_ID`, де `reserve_end_of_month` має найбільше значення та вивести це значення та ПІБ працівника (`full_name`)   Якщо є кілька співробітників із однаковим залишком, то виводити кожного з них. Виводити максимально моюливу кількість записів списком. Якщо не вміщається, поставити в кінці ..., а по наведенню мишки показувати всіх.   Якщо значення в полі відсутнє, то показати текст "Дані відсутні" або знак "-".

Розрахункове поле.   Відібрати `EMPLOYEE_ID`, де `reserve_end_of_month` **за попередній період (місяць)** (дані брати за попередній період, бо за поточний нараховується тільки піля проведення документа нарахування з/п) має найбільше значення та вивести це значення та ПІБ працівника (`full_name`)   Якщо є кілька співробітників із однаковим залишком, то виводити кожного з них.   Якщо значення в полі відсутнє, то показати текст "Дані відсутні" або знак "-".
