...
CALCULATE(
	DIVIDE(
		CALCULATE(
			...
			'fact_Employee_List'[WORK_FORMAT_ON_EMPLOYEE_KEY] = "2"
		),
		COUNTROWS('fact_Employee_List')
	)
)
...
**Бізнес-назва:** Доля онлайн (%)

### Опис із ТЗ

Розрахункове поле: відношення кількості працівників із форматом роботи Онлайн у команді до загальної кількості працівників.   Відношення кількості працівників, для яких `work_format_on_employee_key` = 2 до загальної чисельності команди (метрика Кількість співробітників всього, чол.)

Розрахункове поле: відношення кількості працівників із форматом роботи Онлайн у команді до загальної кількості працівників.   Відношення кількості працівників, для яких `work_format_on_employee_key` = 2 до загальної чисельності команди (людей)
