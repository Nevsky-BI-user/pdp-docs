...
Поле зберігається в довіднику `dm.vw_R27_dim_position`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `position_key` = `position_code_key`.   Поле завжди має значення, пусте поле не допускається   Якщо назва посади не вміщається в одну строку, перенести на іншу

Поле зберігається в довіднику `dm.vw_R27_dim_position`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`, через відповідний зв’язок за ключем `position_key` = `position_code_key`.   Поле завжди має значення, пусте поле не допускається   Якщо назва посади не вміщається в одну строку, перенести на іншу
