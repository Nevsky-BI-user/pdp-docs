# GP.Доля команди з премією за місяць, % факт
...
```mermaid
graph LR
  M["GP.Доля команди з премією за місяць, % факт"]
...
**Бізнес-назва:** Доля команди з премією за місяць, % факт

### Опис із ТЗ

Розрахункове поле.   Потрібно підрахувати кількість працівників у команді для яких є хоча б один запис за останні 12 міс. по `accrual_types_key` = 9df920f3-edfe-5ed9-2386-dc2afbb38bd8, де  поле `PAYMENTS_FACT_UAH`>0 та поділити на поточну кількість членів команди.
