...
VAR _cDelta  = "#23A36B"   // дельта (оцінювач вище самооцінки) — зелений
...
// «Приховані можливості»: лишити компетенцію, якщо хоча б один оцінювач > Самооцінки
VAR _dataF =
    FILTER(
        _data,
        NOT(ISBLANK([Self])) &&
        (
            [Expert] > [Self] || [Mgr] > [Self] || [Peer] > [Self] ||
            [Cross] > [Self] || [Sub] > [Self]
        )
    )
...
VAR _hintText = "Немає прихованих можливостей"
VAR _hintFont = 12
...
VAR _legItems =
    ADDCOLUMNS(
        UNION(
            ...
            ROW("@o", 6, "@lbl", "Вище",        "@col", _cDelta)
        ),
        "@w", _legMarker + _legTextGap + LEN([@lbl]) * _legCharW + _legItemGap
    )
VAR _legNeed = SUMX(_legItems, [@w])
// ширина останнього елемента містить зайвий правий проміжок _legItemGap — віднімаємо для точної правої межі
VAR _legRightEdge = _PadL + _legNeed - _legItemGap

// ── Піксельний розмір полотна (вміщує легенду в один рядок) ──
VAR _contentRight = MAX(MAX(_rightBars, _rightCat), _rightVal) + 4
VAR _W = ROUND(MIN(_MaxW, IF(_isEmpty, _hintW, MAX(_contentRight, _legRightEdge))), 0)

// ── Легенда: один рядок (без переносу) ──
VAR _legTop = 6
VAR _legLineH = 15
VAR _legRows = 1
VAR _legBlockH = _legLineH
VAR _legArranged =
    ADDCOLUMNS(
        _legItems,
...
// ── Легенда (рендер, один рядок зліва) ──
VAR _Legend =
    CONCATENATEX(
        _legArranged,
        VAR _lx = _PadL + [@cumPrev]
        RETURN
            "<rect x='" & FORMAT(_lx, "0.0", "en-US") & "' y='" & _legTop & "' width='" & _legMarker & "' height='" & _legMarker & "' rx='2' fill='" & [@col] & "'/>" &
            "<text x='" & FORMAT(_lx + _legMarker + _legTextGap, "0.0", "en-US") & "' y='" & (_legTop + _legMarker - 1) & "' style='font-family:" & _fontFamily & "; font-size:" & _legFont & "px; fill:" & _legColor & "; font-weight:600;'>" & SUBSTITUTE([@lbl], "&", "&amp;") & "</text>",
        "",
        [@o], ASC
    )
        ...
        VAR _bars =
            CONCATENATEX(
                ...
                VAR _shape =
                    IF(ISBLANK(_v), "",
                        IF(_v > _vSelf,
                            "<rect x='" & FORMAT(_bx, "0.0", "en-US") & "' y='" & FORMAT(_yTop, "0.0", "en-US") & "' width='" & _BarWidth & "' height='" & FORMAT(_selfLineY - _yTop, "0.0", "en-US") & "' fill='" & _cDelta & "'/>" &
                            "<rect x='" & FORMAT(_bx, "0.0", "en-US") & "' y='" & FORMAT(_selfLineY, "0.0", "en-US") & "' width='" & _BarWidth & "' height='" & FORMAT(_BarBot - _selfLineY, "0.0", "en-US") & "' fill='" & _c & "'/>",
