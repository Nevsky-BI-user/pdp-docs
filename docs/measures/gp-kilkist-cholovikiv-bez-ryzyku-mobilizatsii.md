...
*тека `Group_Profile\Загальна інформація` · формат `#,0`*
...
```mermaid
graph LR
  M["GP.Кількість чоловіків БЕЗ ризику мобілізації(%)"]
...
1 - під ризиком. 0 - ризик відсутній

Якщо 1 - Так, якщо 0 - Ні.

Розрахункове поле. Відношення кількості чоловіків під ризиком мобілізації, `is_at_risk` = 1 до загальної кількості чоловіків у команді
...
??? note "Поля-джерела та пов'язані бізнес-метрики (4)"
    ...
    |---|---|
    | `GENDER` | Стать |
    | `IS_AT_RISK` | Під ризиком мобілізації · Ризик мобілізації · Доля чоловіків під ризиком мобілізації (%) |
