# GP.Доля команди із премією за Банк ідей, %
...
```mermaid
graph LR
  M["GP.Доля команди із премією за Банк ідей, %"]
...
**Бізнес-назва:** Доля команди із премією за Банк ідей, %

### Опис із ТЗ

Потрібно підрахувати унікальну кількість працівників у команді на поточний момент, для яких значення `payments_fact_UAH` > 0.00 по  `accrual_types_key` = '99c90425-4ca3-438f-b526-e006f8022f31'    та `category_of_accrual_sort`  = '3' хоча б один раз за останні 12 місяців, НЕ включаючи поточний, та поділити на поточну кількість членів команди.
