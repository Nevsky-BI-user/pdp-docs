# GP.Середній розмір соціальної підтримки
...
Колонки: `Date`, `Index`, `USER_ACCESS_ID`, `category_of_accrual_sort`, `payments_fact_UAH`
...
Колонки: `dim_Admin_LT_OS[USER_ACCESS_ID]`, `dim_Admin_OS[USER_ACCESS_ID]`, `dim_Date[Date]`, `fact_TRS[category_of_accrual_sort]`, `fact_TRS[payments_fact_UAH]`, `t_HierarchyTypes[Index]`

### Схема

```mermaid
graph LR
  M["GP.Середній розмір соціальної підтримки"]
...
**Бізнес-назва:** Середній розмір соціальної підтримки

### Опис із ТЗ

Потрібно зсумувати значення поля `payments_fact_UAH`> 0.00 по тим працівникам, для яких  `category_of_accrual_sort`  = '5' за останні 12 місяців, НЕ включаючи поточний та поділити на кількість людей, які отримали такі виплати   В деталізацію вивести перелік таких працівників та суму по кожному із них
