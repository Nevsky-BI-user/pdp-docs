# PP.Премія за внутрішнє тренерство
...
CALCULATE(
	...
	fact_TRS[accrual_types_key]="050b6a5b-cb83-ef79-a530-00d8adc8e795",
...
```mermaid
graph LR
  M["PP.Премія за внутрішнє тренерство"]
  M --> dim_Date["dim_Date"]
  M --> fact_TRS["fact_TRS"]
```
...
**Бізнес-назва:** Премія за внутрішнє тренерство

### Опис із ТЗ

Потрібно відібрати всі записи по працівнику `person_key`, періоду `Period`, організації `organization_key` , підрозділу `division_key`, посаді `position_key`, де  поле `accrual_types_key` = '050b6a5b-cb83-ef79-a530-00d8adc8e795'   та `category_of_accrual_sort`  = '3' за останні 12 місяців, НЕ включаючи поточний.   Якщо по працівнику записи відсутні, то показати прочерк "-".   В деталізацію винести дані в табличному вигляді суму та місяць, за який було здійснено нарахування
