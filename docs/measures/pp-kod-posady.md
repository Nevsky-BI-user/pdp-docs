...
VAR _res = 
    CALCULATE(
        SELECTEDVALUE(fact_Employee_History_Position[JOB_TITLE_CODE]),
...
Колонки: `JOB_TITLE_CODE`, `PERIOD`

Power Query: `fact_Employee_History_Position`

## Бізнес-суть

JOB_TITLE_CODE → Код ЦБ посади; PERIOD → Дата нарахування премії Зірка МХП; PERIOD → Дата; PERIOD → Період нарахування; PERIOD → Період
...
Колонки: `fact_Employee_History_Position[JOB_TITLE_CODE]`, `fact_Employee_History_Position[PERIOD]`
