...
**Бізнес-назва:** Дійсність інвалідності до

### Опис із ТЗ

Поле зберігається в довіднику ` dm.vw_R27_dim_person`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`  через відповідний зв’язок за ключем `person_key`   Якщо значення в полі відсутнє, то показати текст "Дані відсутні"  або знак "-".
