# AC.Середня кількість лікарняних на співробітника
...
VAR _admin = 
	CALCULATE(
		AVERAGEX( 
			...
			CALCULATE(SUM('fact_Metrics'[sick_leave_id_cnt]))
		),
...
Колонки: `EMPLOYEE_ID`, `IS_MAIN_POSITION`, `Index`, `USER_ACCESS_ID`, `sick_leave_id_cnt`
...
Колонки: `dim_Admin_LT_OS[USER_ACCESS_ID]`, `dim_Admin_OS[USER_ACCESS_ID]`, `fact_Employee_List[EMPLOYEE_ID]`, `fact_Employee_List[IS_MAIN_POSITION]`, `fact_Employee_List[USER_ACCESS_ID]`, `fact_Metrics[sick_leave_id_cnt]`, `t_HierarchyTypes[Index]`

### Схема

```mermaid
graph LR
  M["AC.Середня кількість лікарняних на співробітника"]
...
**Бізнес-назва:** Середня кількість лікарняних на співробітника

### Опис із ТЗ

Розрахункове поле.   Відношення загальної кількості унікальних лікарняних (крім по вагітності та пологам) до кількості тих працівників, у яких були лікарняні, а не всіх членів команди.   Цифру округлювати до цілих в більшу сторону, якщо цифра після коми 5-9, і в меншу сторону, якщо цифра після коми 0-4.

Розрахункове поле.   Відношення загальної кількості унікальних лікарняних (крім по вагітності та пологам) до кількості працівників команди.   Цифру округлювати до цілих в більшу сторону, якщо цифра після коми 5-9, і в меншу сторону, якщо цифра після коми 0-4.

??? note "Поля-джерела та пов'язані бізнес-метрики (6)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `IS_MAIN_POSITION` | Пріоритетне місце роботи · is_main_position |
    | `sick_leave_id_cnt` | #68049 · Кількість лікарняних · Середня кількість лікарняних на співробітника · Лікарняні |
