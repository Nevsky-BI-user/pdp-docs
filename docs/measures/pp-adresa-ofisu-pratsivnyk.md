...
**Бізнес-назва:** Адреса офісу (працівник)

### Опис із ТЗ

Поле зберігається в довіднику `DM.vw_R27_dim_office`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`, через відповідний зв’язок за ключем `office_on_employee_key` = `office_key`.   Якщо значення в полі відсутнє, то показати текст "Дані відсутні"
