...
Колонки: `Calc_Performance_Desc_Rate`, `EMPLOYEE_ID`, `KR_CHANGE`, `USER_ACCESS_ID`
...
Колонки: `dim_Admin_OS[EMPLOYEE_ID]`, `fact_OKR_Goals[Calc_Performance_Desc_Rate]`, `fact_OKR_Goals[EMPLOYEE_ID]`, `fact_OKR_Goals[USER_ACCESS_ID]`, `fact_OKR_Key_Results[KR_CHANGE]`

### Схема

```mermaid
graph LR
  M["PP.OKR.Ціль не виконана"]
...
??? note "Поля-джерела та пов'язані бізнес-метрики (14)"
    ...
    | `KR_CHANGE` | КР змінено · КР без змін · КР видалено · Новий КР · Ознака зміни KR |
