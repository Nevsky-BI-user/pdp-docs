...
**Бізнес-назва:** Кількість основних працівників

### Опис із ТЗ

Поле зберігається в довіднику `DM.vw_R27_dim_Permanent_Temporary`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `permanent_temporary_details_key`.   Деталі підрахунку будуть додані після доробки відповідного довідника та оновлення алгоритму розрахунку.   Потрібно рахувати унікальну к-ть, щоб двічі не рахували тих, в кого осн місце і сумісництво.   Усі хто не Студент, не Стажист і не Сезонний
