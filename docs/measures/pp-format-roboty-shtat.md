...
**Бізнес-назва:** Формат роботи (штат)

### Опис із ТЗ

Поле зберігається в довіднику `DM.vw_R27_dim_Work_Format`    Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`, через відповідний зв’язок за ключем `work_format_on_position_key` = `work_format_key`.    Якщо значення в полі відсутнє, то показати текст "Дані відсутні".
