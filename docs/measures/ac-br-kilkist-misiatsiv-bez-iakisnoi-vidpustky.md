# AC.BR.Кількість місяців без якісної відпустки

*тека `Analytical Cases\Burnout_Risk\Export` · формат `0`*

## Технічний опис

| Властивість | Значення |
|---|---|
| Тип | міра |
| Home table | _Measures |
| displayFolder | `Analytical Cases\Burnout_Risk\Export` |
| formatString | `0` |
| dataType | — |
| Прихована | ні |

### DAX

```dax
SUM(fact_Burnout_Indicators[NO_QUALITY_VACATION_MONTHS])
```

### Джерела даних


Колонки: `NO_QUALITY_VACATION_MONTHS`

Power Query: `fact_Burnout_Indicators`

### Залежності (таблиці й колонки)

Таблиці: `fact_Burnout_Indicators`

Колонки: `fact_Burnout_Indicators[NO_QUALITY_VACATION_MONTHS]`

### Схема

```mermaid
graph LR
  M["AC.BR.Кількість місяців без якісної відпустки"]
  M --> fact_Burnout_Indicators["fact_Burnout_Indicators"]
```

---

## Бізнес-суть

**Бізнес-назва:** Кількість місяців без якісної відпустки

### Опис із ТЗ

Спочатку потрібно визначити чи була у працівника якісна відпустка за останні 36 місяців. Якісна відпустка - це відпустка тривалістю більше 10 робочих днів і більше, підряд, без вихідних за останні 36 місяців. Це може бути відпустка одним наказом або кількома. Наприклад, один наказ на відпустку з 01.09.2025 по 05.09.2025 (5 роб. днів) та інший наказ на відпустку з 08.09.2025 по 12.09.2025 (5 роб. днів) в сумі складе 10 днів відпустки. Це буде якісна відпустка, бо між цими двома відпустками тільки вихідні дні. В аналіз йдуть всі види відпусток, крім відпусток по догляду за дитиною `Vacation_Type_Key` not in (7, 10). Якщо працівник працює на неповну ставку, це не впливає на розрахунок   Потрібно аналізувати тривалість відпустки та часові проміжки між відпустками. Починати розрахунок потрібно з розбивки відпусток на окремі об'єкти для аналізу.   Якщо між відпустками (в проміжку від Дата початку останньої відпустки (`first_day_of_vacation`) та Дати завершення попередньої відпустки (`last_day_of_vacation`)) тільки вихідні дні, то такі відпустки треба об'єднати в одну.   Потім потрібно підрахувати кількість робочих днів у кожній такій відпустці. Якщо робочих днів 10 і більше - відпустка якісна.   Для визначення метрики потрібно брати останню якісну відпустку в періоді та порахувати кількість місяців від поточної дати до дати завершення якісної відпустки. Для звільнених - **від дати звільнення** до дати завершення якісної відпустки. Період розрахунку обмежується датою прийому працівника на роботу (`LAST_ORGANIZATION_HIRE_DATE`). Якщо відпрацював менше 36 місяців, то рахується фактично відпрацьований час і показник не може бути більшим за кількість відпрацьованих місяців.

??? note "Поля-джерела та пов'язані бізнес-метрики (1)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `NO_QUALITY_VACATION_MONTHS` | Кількість місяців без якісної відпустки |

**Вимоги (ТЗ):**

- [Допоміжні вітрини для звіту › Таблиця для розрахунку агрегованих метрик по звіту](https://dev.azure.com/MHPITDepProjects/People%20Digital%20Profile%20%28PDP%29/_wiki/wikis/PDP.wiki?pagePath=/%D0%A4%D1%83%D0%BD%D0%BA%D1%86%D1%96%D0%BE%D0%BD%D0%B0%D0%BB%D1%8C%D0%BD%D1%96%20%D0%B2%D0%B8%D0%BC%D0%BE%D0%B3%D0%B8/%D0%92%D0%B8%D0%BC%D0%BE%D0%B3%D0%B8%20%D0%B4%D0%BE%20%D0%B7%D0%B2%D1%96%D1%82%D1%83%20People%20Digital%20Profile/%D0%94%D0%BE%D0%BF%D0%BE%D0%BC%D1%96%D0%B6%D0%BD%D1%96%20%D0%B2%D1%96%D1%82%D1%80%D0%B8%D0%BD%D0%B8%20%D0%B4%D0%BB%D1%8F%20%D0%B7%D0%B2%D1%96%D1%82%D1%83/%D0%A2%D0%B0%D0%B1%D0%BB%D0%B8%D1%86%D1%8F%20%D0%B4%D0%BB%D1%8F%20%D1%80%D0%BE%D0%B7%D1%80%D0%B0%D1%85%D1%83%D0%BD%D0%BA%D1%83%20%D0%B0%D0%B3%D1%80%D0%B5%D0%B3%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%85%20%D0%BC%D0%B5%D1%82%D1%80%D0%B8%D0%BA%20%D0%BF%D0%BE%20%D0%B7%D0%B2%D1%96%D1%82%D1%83)
- [Кейс Утримання працівників › Опис джерел для сторінки %22Кейс звільнення (вигорання)%22](https://dev.azure.com/MHPITDepProjects/People%20Digital%20Profile%20%28PDP%29/_wiki/wikis/PDP.wiki?pagePath=/%D0%A4%D1%83%D0%BD%D0%BA%D1%86%D1%96%D0%BE%D0%BD%D0%B0%D0%BB%D1%8C%D0%BD%D1%96%20%D0%B2%D0%B8%D0%BC%D0%BE%D0%B3%D0%B8/%D0%92%D0%B8%D0%BC%D0%BE%D0%B3%D0%B8%20%D0%B4%D0%BE%20%D0%B7%D0%B2%D1%96%D1%82%D1%83%20People%20Digital%20Profile/%D0%9A%D0%B5%D0%B9%D1%81%20%D0%A3%D1%82%D1%80%D0%B8%D0%BC%D0%B0%D0%BD%D0%BD%D1%8F%20%D0%BF%D1%80%D0%B0%D1%86%D1%96%D0%B2%D0%BD%D0%B8%D0%BA%D1%96%D0%B2/%D0%9E%D0%BF%D0%B8%D1%81%20%D0%B4%D0%B6%D0%B5%D1%80%D0%B5%D0%BB%20%D0%B4%D0%BB%D1%8F%20%D1%81%D1%82%D0%BE%D1%80%D1%96%D0%BD%D0%BA%D0%B8%20%2522%D0%9A%D0%B5%D0%B9%D1%81%20%D0%B7%D0%B2%D1%96%D0%BB%D1%8C%D0%BD%D0%B5%D0%BD%D0%BD%D1%8F%20%28%D0%B2%D0%B8%D0%B3%D0%BE%D1%80%D0%B0%D0%BD%D0%BD%D1%8F%29%2522)

## На сторінках звіту

[Утримання працівників](../report/utrymannia-pratsivnykiv.md)

## Пов'язані міри

**Використовується в:** [AC.BR.Референтне значення.Місяців без відпустки](../measures/ac-br-referentne-znachennia-misiatsiv-bez-vidpustky.md)

## Нотатки

_порожньо_
