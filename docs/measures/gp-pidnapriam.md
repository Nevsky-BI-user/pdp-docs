...
VAR _admin =
    CALCULATE(
        CONCATENATEX(
            VALUES('dim_Admin_OS'[SUB_DIRECTION]),
...
Колонки: `Index`, `POSITION_CATEGORY_DETAIL`, `SUB_DIRECTION`, `USER_ACCESS_ID`, `USER_ROLE`, `path_length`
...
Колонки: `dim_Admin_OS[POSITION_CATEGORY_DETAIL]`, `dim_Admin_OS[SUB_DIRECTION]`, `dim_Admin_OS[USER_ACCESS_ID]`, `dim_Admin_OS[USER_ROLE]`, `dim_Admin_OS[path_length]`, `t_HierarchyTypes[Index]`
...
Це поле визначається як конкатенація всіх піднапрямів поточного та нижніх рівнів, унікальна (без повторів), через кому   Поле зберігається в довіднику `dm.vw_R27_dim_unit`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `division_key` = `unit_key`.   Поле завжди має значення, пусте поле не допускається   Якщо не вміщається в одну строку, перенести на іншу
