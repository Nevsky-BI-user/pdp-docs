# AC.Switch.Доля взаємодії (Viva) в інтервалі (%)
...
VAR _metric_relevance = 
    CALCULATE(
        ...
        't_Loss_Productivity_Metrics_by_Persona'[Metric_Name] = "Collab_Span_Norm"
    )
VAR _result = 
    SWITCH(
        SELECTEDVALUE('t_AC Burnout'[Burnout_Indicator]),
        "Оцінка", [AC.Оцінка.Доля взаємодії (Viva) в інтервалі (%)],
        "Дані", [AC.Дані.Доля взаємодії (Viva) в інтервалі (%)]
    )
...
```mermaid
graph LR
  M["AC.Switch.Доля взаємодії (Viva) в інтервалі (%)"]
...
**Бізнес-назва:** Доля взаємодії (Viva) в інтервалі (%)
...
**Використовує:** [AC.Дані.Доля взаємодії (Viva) в інтервалі (%)](../measures/ac-dani-dolia-vzaiemodii-viva-v-intervali.md), [AC.Оцінка.Доля взаємодії (Viva) в інтервалі (%)](../measures/ac-otsinka-dolia-vzaiemodii-viva-v-intervali.md)
