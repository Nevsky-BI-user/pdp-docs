...
CALCULATE(
	...
	fact_TRS[category_of_accrual_sort]=5,
...
Колонки: `Date`, `category_of_accrual_sort`, `payments_fact_UAH`
...
Колонки: `dim_Date[Date]`, `fact_TRS[category_of_accrual_sort]`, `fact_TRS[payments_fact_UAH]`

### Схема

```mermaid
graph LR
  M["PP.Соціальна підтримка"]
  M --> dim_Date["dim_Date"]
  M --> fact_TRS["fact_TRS"]
```
...
**Бізнес-назва:** Соціальна підтримка

### Опис із ТЗ

Потрібно відібрати всі записи по працівнику `person_key`, періоду `Period`, організації `organization_key` , підрозділу `division_key`, посаді `position_key`, де  `category_of_accrual_sort` = '5' за останні 12 місяців, НЕ включаючи поточний.   Якщо по працівнику записи відсутні, то показати прочерк "-".   В деталізацію винести дані в табличному вигляді суму, назву виду нарахувань (`type_of_accrual`)  та місяць, за який було здійснено нарахування. Назва виду нарахувань витягти із довідника  `DM`.`vw_R27_dim_Accrual_Type`, через відповідний зв’язок за ключем `accrual_types_key`.
