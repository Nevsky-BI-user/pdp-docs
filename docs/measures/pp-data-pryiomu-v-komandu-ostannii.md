...
LAST_DIVISION_HIRE_DATE → Стаж в команді (останній)  <br>Дата прийому
...
```dax
VAR _res = SELECTEDVALUE('fact_employee_seniority_by_month'[LAST_DIVISION_HIRE_DATE])
RETURN 
	COALESCE(
		FORMAT(
			_res,
...
Колонки: `LAST_DIVISION_HIRE_DATE`
...
Колонки: `fact_employee_seniority_by_month[LAST_DIVISION_HIRE_DATE]`

### Схема

```mermaid
graph LR
  M["PP.Дата прийому в команду (останній)"]
