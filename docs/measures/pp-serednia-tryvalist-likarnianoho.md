...
*тека `Personal_Profile\Здоров'я та благополуччя` · формат `0.0`*
...
**Бізнес-назва:** Середня тривалість лікарняного

### Опис із ТЗ

Розраховується шляхом ділення загальної кількості днів на лікарняному за останні 12 місяців, НЕ включаючи поточний місяць, на кількість унікальних лікарняних листів.    = `sick_leave_day_Total`/`sick_leave_id_cnt`

`sick_leave_day_Total`/`sick_leave_id_cnt`
