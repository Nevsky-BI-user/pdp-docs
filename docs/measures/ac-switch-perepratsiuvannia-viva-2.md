...
**Використовує:** [AC.Дані.Перепрацювання (Viva)](../measures/ac-dani-perepratsiuvannia-viva.md), [AC.Оцінка.Перепрацювання (Viva)](../measures/ac-otsinka-perepratsiuvannia-viva.md)
...
VAR _metric_relevance = 
    CALCULATE(
        ...
        't_Loss_Productivity_Metrics_by_Persona'[Metric_Name] = "After_Hours"
    )
VAR _result = 
    SWITCH(
        SELECTEDVALUE('t_AC Burnout'[Burnout_Indicator]),
        "Оцінка", [AC.Оцінка.Перепрацювання (Viva)],
        "Дані", [AC.Дані.Перепрацювання (Viva)]
    )
