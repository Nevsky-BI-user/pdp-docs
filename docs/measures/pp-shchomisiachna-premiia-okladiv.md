...
BONUS_MONTH_SALARY_CNT → Премія місячна кіл-ть окладів; BONUS_MONTH_SALARY_CNT → Щомісячна премія; BONUS_MONTH_SALARY_CNT → Доля команди з щомісячною премією, %; BONUS_MONTH_SALARY_CNT → Місячна премія; category_name → Назва блоку
...
VAR _v = 
	CALCULATE(
		MAXX(
			'fact_Employee_List',
			'fact_Employee_List'[BONUS_MONTH_SALARY_CNT]
		)
	)
...
//         MAXX(
//             'fact_TRS_Plan',
//             'fact_TRS_Plan'[BONUS_MONTH_SALARY_CNT]
//         ),
...
Колонки: `BONUS_MONTH_SALARY_CNT`, `CALC_TYPE_CODE`, `IS_ACTUAL`, `category_name`
...
Колонки: `fact_Employee_List[BONUS_MONTH_SALARY_CNT]`, `fact_TRS_Plan[BONUS_MONTH_SALARY_CNT]`, `fact_TRS_Plan[CALC_TYPE_CODE]`, `fact_TRS_Plan[IS_ACTUAL]`, `fact_TRS_Plan[category_name]`

### Схема

```mermaid
graph LR
  M["PP.Щомісячна премія окладів"]
