# PP.Щоквартальні премії в середньому за квартал
...
CALCULATE(
	AVERAGEX(
        VALUES('dim_Date'[Quarter and Year]),
        CALCULATE(
...
Колонки: `Date`, `PAYMENTS_FACT_UAH`, `Quarter and Year`, `SUBCATEGORY_OF_ACCRUAL_TYPE`, `TRS_CATEGORY`
...
Колонки: `dim_Date[Date]`, `dim_Date[Quarter and Year]`, `fact_TRS[PAYMENTS_FACT_UAH]`, `fact_TRS[SUBCATEGORY_OF_ACCRUAL_TYPE]`, `fact_TRS[TRS_CATEGORY]`

### Схема

```mermaid
graph LR
  M["PP.Щоквартальні премії в середньому за квартал"]
  M --> dim_Date["dim_Date"]
  M --> fact_TRS["fact_TRS"]
```
...
Якщо по працівнику `person_key`, періоду `Period`, організації `organization_key` , працевлаштуванню `employment_type_key` є значення по полю `ACCRUAL_TYPES_KEY` ='83ce68c2-8a36-d6d5-21bd-27fc6b970114" то Так. Інакше - Ні
...
Відібрати записи за останні 12 місяців де `trs_category` = 'Змінна винагорода'  та `subcategory_of_accrual_type` = 'Щомісячні премії'    В деталізацію вивести назву нарахування, суму та період (місяць) виплати
