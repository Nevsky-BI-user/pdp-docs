...
**Бізнес-назва:** Вид зайнятості

### Опис із ТЗ

Поле зберігається в довіднику `dm.vw_R27_dim_Employment_Type`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`, через відповідний зв’язок за ключем `employment_type_key`.
