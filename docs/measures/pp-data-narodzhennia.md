...
**Бізнес-назва:** Дата народження

### Опис із ТЗ

Поле зберігається в довіднику `dm.vw_R27_dim_person`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `person_key`.   Дата у форматі dd.MM.yyyy Поле Вік - розрахункове. Поточна дата - дата народження.
