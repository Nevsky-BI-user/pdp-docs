# GP.Доля команди із проектним бонусом за стратегічні ІТ проєкти, %
...
```mermaid
graph LR
  M["GP.Доля команди із проектним бонусом за стратегічні ІТ проєкти, %"]
...
**Бізнес-назва:** Доля команди із проектним бонусом за стратегічні ІТ проєкти, %

### Опис із ТЗ

Потрібно підрахувати унікальну кількість працівників у команді на поточний момент, для яких значення `payments_fact_UAH` > 0.00 по  `accrual_types_key` = '8892e575-4037-2f78-ec86-2cdea2b3fbae'   та `category_of_accrual_sort`  = '3' хоча б один раз за останні 12 місяців, НЕ включаючи поточний, та поділити на поточну кількість членів команди.
