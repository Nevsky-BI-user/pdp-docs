...
VAR _admin = 
	CALCULATE(
		DIVIDE(
			SUMX(
				'fact_Metrics',
				'fact_Metrics'[Vacation_Day] - 'fact_Metrics'[WEEKEND_DAY]
			),
...
Колонки: `EMPLOYEE_ID`, `IS_MAIN_POSITION`, `Index`, `USER_ACCESS_ID`, `Vacation_Day`, `WEEKEND_DAY`
...
Колонки: `dim_Admin_LT_OS[USER_ACCESS_ID]`, `dim_Admin_OS[USER_ACCESS_ID]`, `fact_Employee_List[EMPLOYEE_ID]`, `fact_Employee_List[IS_MAIN_POSITION]`, `fact_Employee_List[USER_ACCESS_ID]`, `fact_Metrics[Vacation_Day]`, `fact_Metrics[WEEKEND_DAY]`, `t_HierarchyTypes[Index]`

### Схема

```mermaid
graph LR
  M["AC.Відсоток днів відпустки в робочі дні"]
...
Показує відсоток днів відпуски, які припадають на робочі дні за останні 12 місяців, включно із поточним.   Розраховується як (`Vacation_Day`-`Weekend_Day`)/`Vacation_Day`   Якщо значення в полі відсутнє, то показати текст "Дані відсутні".

Розрахункове поле.   Кількість днів відпусток всіх видів, використаних за останні 12 місяців (включно із поточним місяцем) в сумі.

Розрахункове поле.   Потрібно кількість днів відпустки за останні 12 місяців поділити на кількість таких відпусток та округлити до десятих = `Vacation_Day`/`Vacation_Number`   Якщо значення в полі відсутнє, то показати текст "Дані відсутні" або знак "-".

Розрахункове поле.   Відношення загальної кількості днів використаних відпусток членів команди за останні 12 місяців, включно із поточним, до загальної кількості членів команди.

??? note "Поля-джерела та пов'язані бізнес-метрики (9)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `IS_MAIN_POSITION` | Пріоритетне місце роботи · is_main_position |
    | `Vacation_Day` | % днів відпустки  в робочі дні (за останні 12 місяців) · Дні відпустки · Середня тривалість відпустки · Середня кількість днів використаної відпустки співробітником 12 міс. · Середня тривалість відпустки співробітника, днів · % днів відпустки  в робочі дні · % днів відпустки в робочі дні |
