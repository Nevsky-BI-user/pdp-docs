...
VAR _MaxVal = MAXX( ALLSELECTED('fact_Employees_Attributes'[job_title_composite_vehicle_format]), [GP.Кількість співробітників всього, чол. - Integer] )

VAR _Data = 
    ADDCOLUMNS(
        VALUES('fact_Employees_Attributes'[job_title_composite_vehicle_format]),
        "@Val", [GP.Кількість співробітників всього, чол. - Integer]
    )

VAR _FilteredData = FILTER(_Data, NOT(ISBLANK('fact_Employees_Attributes'[job_title_composite_vehicle_format])) && [@Val] > 0)
...
VAR _DataWithRank = 
    GENERATE(
        _FilteredData,
        VAR _CurrVal = [@Val]
        VAR _CurrCat = 'fact_Employees_Attributes'[job_title_composite_vehicle_format]
        VAR _Rank = 
            COUNTROWS(
                FILTER(
                    _FilteredData,
                    [@Val] > _CurrVal || ([@Val] = _CurrVal && 'fact_Employees_Attributes'[job_title_composite_vehicle_format] <= _CurrCat)
                )
            )
...
VAR _SVG_Bars = 
    CONCATENATEX(
        ...
        VAR _Label = 'fact_Employees_Attributes'[job_title_composite_vehicle_format]
...
Колонки: `job_title_composite_vehicle_format`
...
Колонки: `fact_Employees_Attributes[job_title_composite_vehicle_format]`

### Схема

```mermaid
graph LR
  M["GP.Опція по авто, % план.SVG"]
...
??? note "Поля-джерела та пов'язані бізнес-метрики (3)"
    | Поле | Бізнес-метрики |
    |---|---|
    | `job_title_composite_vehicle_format` | Тип опції по авто по зведеній посаді · Тип опції по авто по зведеній посаді (план) · Опція по авто план |
