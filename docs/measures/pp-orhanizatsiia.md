...
**Бізнес-назва:** Організація

### Опис із ТЗ

`organization_key`=`organization_id`

Поле зберігається в довіднику `dm.vw_R27_dim_organization`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List`, через відповідний зв’язок за ключем `organization_key`.   Якщо назва організації не вміщається в одну строку, перенести на іншу

Поле зберігається в довіднику `dm.vw_R27_dim_organization`   Це поле має бути доступне у візуалізаціях, побудованих на основі фактової таблиці `dm.vw_R27_fact_Employee_List_PDP`, через відповідний зв’язок за ключем `organization_key`.   Якщо назва організації не вміщається в одну строку, перенести на іншу
