...
Вихідні таблиці: `DM.vw_R27_dim_Employee_Access_List`, `DM.vw_R27_dim_Position`, `DM.vw_R27_fact_Employee_List`, `DWH.t_SPO_HR_Group_Profile_General_Metric`

Колонки: `CLOSE_VACANCY_COUNT`, `DIRECTION`, `Direction_Name`, `EMPLOYEE_EMAIL`, `Index`, `ORDER_NUM_2`, `POSITION_CATEGORY_DETAIL`, `Record_Type`, `SUB_DIRECTION`, `Staffing`, `TOTAL_VACANCY`, `USER_ACCESS_ID`

Power Query: `dim_Admin_OS`

### Залежності (таблиці й колонки)

Таблиці: `dim_Admin_OS`, `fact_EXCEL_Group_Profile_General_Metric`, `fact_Employee_List`, `fact_Vacancy`, `t_HierarchyTypes`

Колонки: `dim_Admin_OS[DIRECTION]`, `dim_Admin_OS[EMPLOYEE_EMAIL]`, `dim_Admin_OS[ORDER_NUM_2]`, `dim_Admin_OS[POSITION_CATEGORY_DETAIL]`, `dim_Admin_OS[SUB_DIRECTION]`, `fact_EXCEL_Group_Profile_General_Metric[Direction_Name]`, `fact_EXCEL_Group_Profile_General_Metric[Record_Type]`, `fact_EXCEL_Group_Profile_General_Metric[Staffing]`, `fact_Employee_List[USER_ACCESS_ID]`, `fact_Vacancy[CLOSE_VACANCY_COUNT]`, `fact_Vacancy[TOTAL_VACANCY]`, `t_HierarchyTypes[Index]`

### Схема

```mermaid
graph LR
  M["GP.Укомплектованість штату (%)"]
  M --> dim_Admin_OS["dim_Admin_OS"]
  M --> fact_EXCEL_Group_Profile_General_Metric["fact_EXCEL_Group_Profile_General_Metric"]
  M --> fact_Employee_List["fact_Employee_List"]
  M --> fact_Vacancy["fact_Vacancy"]
...
**Бізнес-назва:** Укомплектованість штату (%)

### Опис із ТЗ

Значення виводити у відсотках %

Розрахункове поле.   **Укомплектованість штату** = фактична кількість працівників (зайнятих ставок>0) включно із аутсорсом команди / штатна чисельності підрозділу без вакансій на майбутнє (Планова дата виходу <=поточній даті), включно з аутсорсом. **Фактична кількість працівників** = `FTE_employee`>0 (dm.`vw_R27_fact_Employee_List_PDP`) - FTE прац-в у станах Мобілізований та Декрет (`status_key` = 2 або 3) + FTE аутсорса (`is_outsourcing` = 1). Вважати що аутсорос - завжди зайнята посада. **Штатна чисельність** = Із dm.`vw_R27_fact_Employee_List` відібрати кількість FTE (`position_amt`) по організації (`organization_key`) та підрозділу (`division_key`) за відповідний період де поле `plan_go_work_date`<=поточна дата, включно з аутсорсом та вакансіями. Якщо обрано **Всі підлеглі** - Укомплектованість  рахується по ієрархії кадрових підрозділів до низу. Якщо обрано **Моя команда** - в чисельнику буде кількість зайнятих посад, які підпорядковуються керівнику. В знаменнику - кількість посад, які підпорядковуються керівнику. Ця частина задачі буде реалізована окремо.
...
??? note "Поля-джерела та пов'язані бізнес-метрики (12)"
    ...
    | `Record_Type` | Тип винятку із плинності |
    | `SUB_DIRECTION` | Піднапрям · subdirection · sub_direction |
    | `Staffing` | Укомплектованість штату (%) |
